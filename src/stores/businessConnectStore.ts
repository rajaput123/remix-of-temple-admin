...
import { getAccount } from "@/lib/bcAuthService";
...
export function syncBCSession(mobile: string, businessName: string, businessType: string) {
  bcStore.set({
    account: { mobile, verified: true, registered: true, mpinSet: true },
...
export function syncBCLogin(mobile: string) {
  const account = getAccount(mobile);
  if (!account) return;
  bcStore.set({
    account: { mobile, verified: true, registered: true, mpinSet: true },
    businessType: { category: account.businessType },
    info: {
