import { Eye, Globe, Mail, MapPin, Pencil, Phone, Send, ShieldCheck } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import type { BusinessProfile } from "@/types/businessProfile";
import { BUSINESS_TYPES } from "@/data/businessTypes";
import { businessTypeLabel, formatOptionalText, formatUpdatedAt, profileDisplayName, profileEntityLabel, shouldShowGst } from "@/components/business-profile/profileUtils";
          ...
          <div className="-mt-14 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex items-end gap-4">
              ...
              <div className="min-w-0 pb-1">
                ...
                <h2 className={t.title}>{displayName}</h2>
                <p className={t.muted}>
        ...
        <Card className={profileCardClass}>
          ...
          <CardContent className="px-5 pb-5 space-y-3 text-sm">
            <p className="flex items-center gap-2">
              <Phone className="h-4 w-4 text-primary" /> {formatOptionalText(profile.mobile, "—")}
            </p>
            <p className="flex items-center gap-2">
              <Phone className="h-4 w-4 text-primary" /> WhatsApp: {formatOptionalText(profile.whatsapp, "—")}
            </p>
            <p className="flex items-center gap-2">
              <Mail className="h-4 w-4 text-primary" /> {formatOptionalText(profile.email, "Not provided")}
            </p>
            <p className="text-muted-foreground">
              {profile.entityType === "company" ? "Contact" : "Name"}: {formatOptionalText(profile.ownerName, "—")}
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent className="px-5 pb-5 grid grid-cols-1 gap-4 sm:grid-cols-3 text-sm">
            ...
            {shouldShowGst(profile) && (
              <div className="rounded-xl bg-muted/40 p-3 ring-1 ring-black/[0.04]">
                ...
                <p className="mt-1 font-medium">
                  {profile.gst.trim() ||
                    (profile.entityType === "individual" ? "Not registered" : "Not provided")}
                </p>
              </div>
            )}
          </CardContent>
